...
class EdgeSyncService {
  ...
  async sendVideoMetadata(baseUrl, apiKey, videoId, videoPath, videoDir) {
    ...
    const response = await fetch(`${baseUrl}/api/edge/video/metadata`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to send metadata: ${response.status} - ${errorText}`);
    }
  }
  ...
  async uploadVideoFiles(baseUrl, apiKey, videoId, videoDir) {
    ...
    const response = await fetch(`${baseUrl}/api/edge/video/files`, {
      method: 'POST',
      headers: {
        'X-API-Key': apiKey,
        ...formData.getHeaders()
  ...
  async checkEdgeServerHealth(edgeServer) {
    try {
      const baseUrl = `${edgeServer.protocol}://${edgeServer.host}:${edgeServer.port}`;
      const response = await fetch(`${baseUrl}/api/edge/health`, {
        method: 'GET',
        headers: {
